...
def extract_clauses_flexible(text: str):
    ...
    for i in range(len(matches)):
        ...
        clauses.append({"clause_id": clause_id, "clause_title": clause_title, "clause_body": clause_body})
...
def extract_structured_sections(text: str, doc_type: str) -> List[Dict]:
    ...
    for i, match in enumerate(matches):
        ...
        structured_sections.append(
            {"section_id": section_id, "section_title": section_title, "section_body": section_body}
        )

    # If no structured sections found, create artificial ones
    if not structured_sections:
        logger.info("legacy_log", message=str("⚠️ No structured sections found. Creating artificial sections."))
...
def create_artificial_sections(text: str, section_size: int = 1000) -> List[Dict]:
    ...
    for para in paragraphs:
        if len(current_section) + len(para) > section_size and current_section:
            ...
            sections.append({"section_id": str(section_id), "section_title": title, "section_body": current_section})
            section_id += 1
...
def chunk_by_paragraphs(section: Dict, max_tokens: int, overlap_tokens: int) -> List[Dict]:
    ...
    for para in paragraphs:
        ...
        if current_tokens + para_tokens > max_tokens and current_chunk:
            # Create chunk
            chunks.append(create_chunk(section, current_chunk, len(chunks), extract_universal_keywords(current_chunk)))
...
def chunk_by_tokens(section: Dict, max_tokens: int, overlap_tokens: int) -> List[Dict]:
    ...
    while start < total_tokens:
        ...
        chunks.append(create_chunk(section, chunk_text, len(chunks), extract_universal_keywords(chunk_text)))
...
def create_chunk(section: Dict, text: str, chunk_index: int, keywords: List[str], doc_id: str = "unknown") -> Dict:
...
def load_or_create_cache(url: str, raw_text: str, doc_type: str) -> Tuple[List[Dict], np.ndarray, str]:
    ...
    if chunks_path.exists() and embeddings_path.exists():
        logger.info("legacy_log", message=str(f"✅ Using cached chunks and embeddings for {url}"))
        chunks = pickle.load(open(chunks_path, "rb"))
        embeddings = np.load(embeddings_path)
    else:
        logger.info("legacy_log", message=str(f"⚡ Creating new chunks and embeddings for {url}"))
...
def reduce_dimensions(vectors: np.ndarray, target_dim: int = 512) -> Tuple[np.ndarray, PCA]:
    ...
    logger.info(
        "legacy_log",
        message=str(f"[PCA] d={n_features} → {td}, retained {sum(pca.explained_variance_ratio_)*100:.2f}% variance."),
    )
    return reduced.astype("float32"), pca


# ------------------------------------------------------------------------------
# 3. NumPy Index Builder
# ------------------------------------------------------------------------------
...
def filter_candidates(inv_index: Dict[str, Set[int]], query: str, total: int, verbose=False) -> List[int]:
    ...
    if verbose:
        logger.info("legacy_log", message=str(f"[Keyword Filter] Query: '{query}' → Keywords: {kws}"))
        logger.info("legacy_log", message=str(f"[Keyword Filter] Candidates: {len(cand)} chunks"))
    return list(cand)
...
def calculate_universal_scores(
    query: str, chunk: Dict, query_emb: np.ndarray, chunk_emb: np.ndarray, doc_type: str
) -> Dict[str, float]:
...
def get_adaptive_contextual_chunks(
    selected_chunks: List[Dict], all_chunks: List[Dict], doc_type: str, context_strategy: str = "adaptive"
) -> List[Dict]:
    ...
    for attempt in range(max_retries):
        try:
            ...
            if parsed_answers:
                ...
                logger.info("legacy_log", message=str(f"⚠️ Attempt {attempt + 1}: Failed to parse JSON, retrying..."))
        except Exception as e:
            logger.info("legacy_log", message=str(f"⚠️ Attempt {attempt + 1} failed: {str(e)}"))
            if attempt == max_retries - 1:
                logger.info("legacy_log", message=str("🔴 All attempts failed, using fallback extraction"))
...
def extract_answers_fallback(content: str) -> List[str]:
    ...
    for text in quoted_text:
        if any(
            ...
            for keyword in ["period", "coverage", "policy", "days", "months", "years", "benefit", "claim"]
        ):
            answers.append(text)
    ...
    if not answers:
        ...
        for sentence in sentences:
            if len(sentence.strip()) > 20 and any(
                ...
                for keyword in ["period", "coverage", "policy", "waiting", "claim", "benefit"]
            ):
...
def test_json_parsing():
    ...
    for i, test in enumerate(test_responses):
        ...
        logger.info("legacy_log", message=str(f"Test {i+1}: {result}"))
...
def lightweight_reranking(query_processed: Dict, retrieved_chunks: List[Dict], top_k: int = 5) -> List[Dict]:
    """
...
def load_or_create_chunks(file_url: str) -> Tuple[List[Dict], np.ndarray, str, str]:
    ...
    """
    logger.info("legacy_log", message=str("🌍 [Universal RAG] Starting document processing"))
    ...
    logger.info("legacy_log", message=str(f"✅ Text extracted: {len(raw_text)} characters"))
    # Clean text
    cleaned_text = clean_text(raw_text)

    # Detect document type
    doc_type = DocumentClassifier.classify_document(cleaned_text[:5000])
    logger.info("legacy_log", message=str(f"🎯 Document type detected: {doc_type.upper()}"))
    # Generate cache paths
    chunks_path = CACHE_DIR / f"{doc_id}_chunks.pkl"
    embeddings_path = CACHE_DIR / f"{doc_id}_embeddings.npy"

    if chunks_path.exists() and embeddings_path.exists():
        logger.info("legacy_log", message=str(f"✅ Using cached chunks and embeddings for {file_url}"))
        chunks = pickle.load(open(chunks_path, "rb"))
        embeddings = np.load(embeddings_path)
    else:
        logger.info("legacy_log", message=str(f"⚡ Creating new chunks and embeddings for {file_url}"))
        # Extract structured sections and chunk
        sections = extract_structured_sections(cleaned_text, doc_type)
        for section in sections:
            section["doc_id"] = doc_id
        logger.info("legacy_log", message=str(f"📄 Sections extracted: {len(sections)}"))
        ...
        logger.info("legacy_log", message=str(f"🧩 Chunks created: {len(chunks)}"))
        embeddings = embed_voyage([c["text"] for c in chunks])
        pickle.dump(chunks, open(chunks_path, "wb"))
        np.save(embeddings_path, embeddings)

    logger.info("legacy_log", message=str(f"⏱️ Total processing time: {time.time() - start_total:.2f} seconds"))
...
def handle_queries(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(_process_single_query, query, chunks, chunk_embeddings, inv_index, doc_type, doc_id, top_k)
